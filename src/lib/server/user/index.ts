...
export class UserService {
	...
	public async register(email: string, password: string) {
		...
		let pending = new PendingRegistration(email, passwordHash);
	...
	public async confirmEmail(
		...
		await this.repos.user.create(pending.email, pending.passwordHash);
...
export class UserRepository extends Repository {
	...
	public async create(email: string, passwordHash: string): Promise<void> {
		await this.db.insertInto("user").values({ email, passwordHash }).execute();
	}
}
